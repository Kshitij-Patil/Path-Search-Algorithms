...
Uniform::Uniform(const Graph USGraph, const string source, const  string destination)
{
...
void Uniform::ExecutePathSearchAlgorithm()
{
		...
		else
		{
			...
			for (succ_iterator = successor_list.begin(); succ_iterator != successor_list.end(); ++succ_iterator)
			{
				...
				if (find(ExpandedNodes.begin(), ExpandedNodes.end(), succ_iterator->first) == ExpandedNodes.end())
				{
					...
					estimated_cost_data_structure[current_path] = updated_cost + heuristic_distance;
				}
			}
...
inline float Uniform::EstimateHeuristic(string start, string end)
{
...
vector<string> Uniform::ShiftMinCostElementToTop(map <vector<string>, float> estimated_cost_data_structure)
{
...
void Uniform::RemoveDuplicatePaths()
{
